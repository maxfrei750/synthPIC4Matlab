...
rayOrigin = [50 50 10e3];
% rayOrigin = single(rayOrigin);
...
rayDirections_in = double(rayDirections_in);
...
intersectionDistancesArray = gather(double(intersectionDistancesArray));

transmissionDistances = max(intersectionDistancesArray,[],2)-min(intersectionDistancesArray,[],2);
transmissionDistances = reshape(transmissionDistances,100,100);

imagesc(transmissionDistances)